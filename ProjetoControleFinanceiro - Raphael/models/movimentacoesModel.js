...
const getMovimentacoesPorUsuario = async (id_usuario) => {
    const query = 'SELECT m.id, m.descricao, m.valor, m.tipo, m.data_movimentacao, m.id_usuario, m.id_tipo_pagamento, u.nome, tp.nome_tipo_pagamento FROM movimentacoes m JOIN usuarios u ON m.id_usuario = u.id JOIN tipo_pagamentos tp ON m.id_tipo_pagamento = tp.id_tipo_pagamento WHERE u.id = $1';
...
//h) Busca dados para gráfico de tipo de pagamento
const buscarDadosGraficoTipoPagamento = async (id) => {
    const query = `
        SELECT
            tp.nome_tipo_pagamento AS nome_tipo_pagamento,
            SUM(m.valor) AS total_gasto
        FROM
            movimentacoes AS m
        JOIN
            tipo_pagamentos AS tp ON m.id_tipo_pagamento = tp.id_tipo_pagamento
        WHERE
            m.id_usuario = $1 
        GROUP BY
            tp.nome_tipo_pagamento
        ORDER BY
            total_gasto DESC;
    `;
...
const buscarDadosGraficoTipoMovimentacao = async (id) => {
    const query = `
    SELECT
...
module.exports = {
    ...
    excluirMovimentacao,
    buscarDadosGraficoTipoPagamento,
    buscarDadosGraficoTipoMovimentacao
};
